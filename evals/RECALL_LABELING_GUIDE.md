...
Read the full diff and linked commit, then set exactly one `adjudication`:
...
An empty adjudication means unjudged. Never turn it into `clean` merely because `missed` is empty.
...
Re-running `recall-probe` with the same source manifest, seed, count, and output path preserves matching adjudications. If the source review manifest changes, use a new output path or archive the old probe; the tool intentionally refuses to imply that two different source runs are the same experiment.
